...
- 카드 미리보기 = **사용 예시 시안(`<프레임명>_sample.png`)** — 사진이 채워진 완성 예시가 빈
  프레임보다 템플릿의 매력을 잘 전달한다 (참고 시안과 동일한 접근)
- 파생 스크립트가 1080폭으로 축소해 `public/frames/<id>/preview.webp` 생성 (스펙 01)
...
- E2E: 홈 → 카드 6장 확인 → 첫 카드 탭 → `/editor/duo` 도착, 미존재 frameId 404,
