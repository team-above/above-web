...
- 화질 2차 (2026-08-07, 실기기 제보 — 작은 슬롯 에일리어싱): 첨부 시 2160px로 디코드된
  사진을 Weekly Dump 232px 칸에 그리면 **한 방 7~9배 축소**가 일어나, 고주파 사진(진열대·
  텍스트)이 자글자글 깨진다. 브라우저 단일 패스 리사이즈는 drawImage든 createImageBitmap
  (resizeQuality "high")든 대배율에서 픽셀을 건너뛴다(실측 13배: 에너지 151 vs 점진 반감 33
  vs 이상적 29). 조치: **해상도 관리를 양방향으로** — 필요 폭(사진폭×scale)보다 과대하면
  점진 반감(downscaleBitmap, 2배씩)으로 필요 폭 ×1까지 하향(fitPhoto), 부족하면 원본 풀
  디코드 후 점진 반감으로 상향(upgradePhoto). 히스테리시스(상향 1.05 / 하향 ×1.5)로 재디코드
  핑퐁 방지. 검증: 하향 비트맵이 이상적 축소본과 픽셀 동급(에너지 22=22), 텍스트 실사에서
  옛 경로 42 → 새 경로 23(이상적 27). 부수효과: 7칸 만재 시 비트맵 메모리 98MB → 8MB
...
- 파일 선택 취소 → 아무 일 없음
- 슬롯 형상(radius·마스크)과 무관하게 placement rect 전체가 탭 히트 영역
...
2. punching(별 오버레이)·weeklydump(라벨·그라디언트)·doodle(낙서 겹침)에 사진을 넣으면 샘플과
